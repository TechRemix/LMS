  ...
  document.querySelectorAll(".apply-role-btn").forEach(button => {
    button.addEventListener("click", async () => {
      ...
      const res = await fetch("php/toggle-status.php", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: `id=${encodeURIComponent(userId)}&status=approved&role=${encodeURIComponent(selectedRole)}`
      });
      ...
      if (result.success) {
        ...
        if (user) {
          user.role = selectedRole;
          user.status = "approved"; // enforce approved state after applying role
        }

        // ✅ Re-render just this user + any remaining pending users
        const visibleUsers = allUsers.filter(u => u.status === "pending" || u.id == userId);
        renderUserTable(visibleUsers);
      } else {
...
if (!fuse) {
  fuse = new Fuse(allUsers, {
    ...
    threshold: 0.5,
    includeMatches: true   // ✅ this enables match highlighting
  });
}
